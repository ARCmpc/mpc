...
a Python wrapper for a fast solver generated by FORCES Pro
   ...
   OUTPUT = arc_solver_py.arc_solver_solve(PARAMS) solves a multistage problem
   ...
   [OUTPUT, EXITFLAG] = arc_solver_py.arc_solver_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = arc_solver_py.arc_solver_solve(PARAMS) returns 
...
#_lib = ctypes.CDLL(os.path.join(os.getcwd(),'arc_solver/lib/arc_solver.so')) 
try:
	_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'arc_solver/lib/arc_solver.so'))
	csolver = getattr(_lib,'arc_solver_solve')
except:
	_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'arc_solver/lib/libarc_solver.so'))
	csolver = getattr(_lib,'arc_solver_solve')

class arc_solver_params_ctypes(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
	_fields_ = [('x0', ctypes.c_double * 54),
('xinit', ctypes.c_double * 4),
('all_parameters', ctypes.c_double * 27),
]

arc_solver_params = {'x0' : np.array([]),
'xinit' : np.array([]),
...
class arc_solver_outputs_ctypes(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
	_fields_ = [('x1', ctypes.c_double * 6),
('x2', ctypes.c_double * 6),
('x3', ctypes.c_double * 6),
('x4', ctypes.c_double * 6),
('x5', ctypes.c_double * 6),
('x6', ctypes.c_double * 6),
('x7', ctypes.c_double * 6),
('x8', ctypes.c_double * 6),
('x9', ctypes.c_double * 6),
]

arc_solver_outputs = {'x1' : np.array([]),
'x2' : np.array([]),
'x3' : np.array([]),
'x4' : np.array([]),
'x5' : np.array([]),
...
class arc_solver_info(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
...
csolver.argtypes = ( ctypes.POINTER(arc_solver_params_ctypes), ctypes.POINTER(arc_solver_outputs_ctypes), ctypes.POINTER(arc_solver_info), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

def arc_solver_solve(params_arg):
	'''
a Python wrapper for a fast solver generated by FORCES Pro
 ...
 See also COPYING
	...
	# convert parameters
	params_py = arc_solver_params_ctypes()
	...
	outputs_py = arc_solver_outputs_ctypes()
	info_py = arc_solver_info()
	if sys.version_info.major == 2:
		...
		try:
			PyFile_AsFile.restype = ctypes.POINTER(FILE)
			exitflag = _lib.arc_solver_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp) )
		...
		try:
			if sys.platform.startswith('win'):
				exitflag = _lib.arc_solver_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), None )
			else:
				exitflag = _lib.arc_solver_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), fp )
			libc.fclose(fp)
	...
	# convert outputs
	for out in arc_solver_outputs:
		arc_solver_outputs[out] = npct.as_array(getattr(outputs_py,out))

	return arc_solver_outputs,int(exitflag),info_py

solve = arc_solver_solve
